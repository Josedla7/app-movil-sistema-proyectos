...
import { ProjectHistoryItem } from '../../api/history.service';
...
const ClientHistoryScreen: React.FC = () => {
  ...
  const loadHistory = async (isRefresh = false) => {
    try {
      ...
      if (searchQuery) {
        ...
        response = await historyService.getProjectHistory(Number(user?.id) || 0, isRefresh ? 1 : page);
      }
  ...
  const loadProyectos = async () => {
    try {
      const response = await historyService.getProyectos('/proyectos/mis-proyectos');
  ...
  const renderHistoryItem = ({ item }: { item: ProjectHistoryItem }) => (
    <View style={styles.historyItem}>
      {/* Entidad principal */}
      <View style={styles.projectHeader}>
        <Briefcase size={16} color="#3b82f6" />
        <Text style={styles.projectName}>{item.proyecto_nombre}</Text>
        <Text style={styles.projectStatus}>#{item.id}</Text>
      </View>
      ...
      <View style={styles.footerRow}>
        <View style={styles.userInfo}>
          ...
          <Text style={[styles.textSm, styles.textSlate400]}>{item.usuario_nombre}</Text>
        </View>
        <Text style={[styles.textXs, styles.textSlate500]}>
          {dayjs(item.fecha_creacion).format('DD MMM YYYY, HH:mm')}
        </Text>
      </View>
    </View>
  );
